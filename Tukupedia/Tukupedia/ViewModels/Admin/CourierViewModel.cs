...
namespace Tukupedia.ViewModels.Admin
{
    class CourierViewModel
    {
        ...
        void reload()
        {
            cm.initAdapter($"select KODE as \"Kode\", NAMA as \"Nama Kurir\", to_char(HARGA) as \"Harga /KM\", case STATUS when '1' then 'Aktif' else 'Non Aktif' end as \"Status Kurir\" from KURIR where STATUS = '1' order by KODE");
